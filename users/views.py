...
class StatGithubAPIView(APIView):
    def get(self, request):
        ...
        avg_repo = (
            round(count_repo / count_author, 1) if (count_repo and count_author) else 0
        )
        return Response(
            {
                "Number of users (projects)": count_author,
...
class StatRepoGithubAPIView(APIView):
    def get(self, request):
        ...
        content = {}
        for i in users_lst:
            lst = Repo.objects.filter(url_user__exact=i["url"]).values_list(
                "name_repo", "commits"
            )
            m = max(lst, key=lambda x: x[1]) if lst else [None, 0]
            avg_stars = Repo.objects.filter(url_user__exact=i["url"]).aggregate(
                avg=Avg("stars")
            )
            content[i["url"]] = {
                "Repository with the maximum number of commits": m,
                "Average number of stars in repositories": avg_stars["avg"]
                if avg_stars["avg"] is not None
                else 0,
            }
        return Response(content)
